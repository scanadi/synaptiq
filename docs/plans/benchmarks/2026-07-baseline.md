...
**Update (W4.1): pyarrow is now a core dependency**, so the Arrow COPY path these
numbers were measured with is the default — a plain `pip install synaptiq` now
gets the "Loading to storage" numbers above out of the box. (The `fast-load`
extra remains as a deprecated no-op alias for install-command compatibility.)
